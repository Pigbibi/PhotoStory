...
export default function History({api}){
 ...
 return <section className="settings-panel">
  ...
  {failed&&<p role="alert">{t('request_failed')}</p>}
  <button disabled={busy} onClick={async()=>{setBusy(true);setFailed(false);try{setData(await api('/api/history/sync','POST',{}));}catch{setFailed(true);}finally{setBusy(false);}}}>{t('Read Instagram history page')}</button>
  {data&&<>
