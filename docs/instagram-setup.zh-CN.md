...
Meta 可能调整控制台界面名称、权限名称和准入要求；以你账户中的官方页面为准。
...
部署时一并更新 Worker 及 `scripts/` 下全部 Python 文件（包括
...
低频部署建议每周检查一次最近三个月的照片，每次分析上限可配置（默认 300 张）。没有新照片或没有通过严格审核的草稿时不发布、不凑数量；未使用的发布机会不累积。
