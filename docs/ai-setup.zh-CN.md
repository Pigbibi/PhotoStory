...
处理器会通过现有隔离 AI 服务，将标题和选片说明翻译成界面支持的 13 种语言。
此步骤只发送文字，不重新发送照片。切换界面语言只切换显示译文，不修改原始主题、
发布文案、图片顺序、构图、版本或批准状态。手动编辑主题会清除旧译文并要求重新批准；
缺少译文时显示原文。`AI_CAPTION_LANGUAGE` 仍独立决定发布文案语言。

已有草稿可由管理员导出私有的 `id`、`title`、`reason` JSON 数组，配置原有
`CODEX_GATEWAY_COMMAND` 后执行 `scripts/translate_labels.py --input labels.json --output translated.json`，
每次最多 5 条。确认原始标题、说明和版本未变化后，只合并对应记录的 `translations`
字段，不覆盖整条已批准草稿。JSON 文件不得提交仓库，验证后清理临时副本。
纯文字翻译需要同时更新两份隔离网关脚本，不需要新密钥或更换 AI 服务。
