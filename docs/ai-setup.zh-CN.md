...
升级时将 `preselect.py`、`process_batch.py` 和 `inventory.py` 一起部署，复用已有 Pillow。
先扫描完整日期范围元数据，再分批读取缩略图。在本地结合双重感知哈希、比例、颜色和对比度，
保守过滤两分钟内的相似连拍；低细节照片不单靠哈希合并。同批优先保留清晰代表，后续明显
更清晰的照片仍保留。不确定或不同角度继续交给 AI。这是启发式筛选，不保证识别全部重复或
准确评价美感，隐私与原图裁切复核仍执行。每组评分最高的照片优先作为封面，同分保留 AI 排序。

只有实际送入 AI 筛选的照片计入额度，新配置默认 300 张。服务端确认批次后，已处理版本和
代表特征保存在 VPS 私有缓存，跨批、跨周期使用；不存图片像素或令牌。旧缓存继续有效，
不会因升级全部重扫。结果不明的批次不重放。

网站定期制作可保存最近一个月、三个月、半年、一年、全部照片、自定义起止日期，或从指定
日期起到今天。固定起点模式每次将结束日期推进到当天，适合分周补扫历史；固定起止日期只
处理该区间。已有部署保留保存过的额度，需在网站调整。保存后在下个计划周期执行，不会
立即把全部历史送给 AI，也不增加发帖频率。
