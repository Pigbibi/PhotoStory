...
export async function begin(e,id,publicationId,version,username){
 ...
 const result=await e.DB.prepare("UPDATE publications SET status='publishing',expires=? WHERE id=? AND status='prepared' AND NOT EXISTS(SELECT 1 FROM publications other,json_each(other.body,'$.photos') old JOIN json_each(publications.body,'$.photos') current ON json_extract(old.value,'$.id')=json_extract(current.value,'$.id') WHERE other.id<>publications.id AND other.status IN ('published','publishing','working','uncertain')) AND EXISTS(SELECT 1 FROM drafts WHERE id=? AND version=? AND json_extract(body,'$.status')='approved') AND (?=0 OR EXISTS(SELECT 1 FROM state WHERE key='automation' AND json_extract(value,'$.publishMode')='automatic' AND json_extract(value,'$.reviewMode')='strict_auto' AND json_extract(value,'$.autoPublishSince')=? AND json_extract(value,'$.autoPublishUserId')=?))")
