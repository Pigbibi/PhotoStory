...
export async function summary(e){
 ...
 let instagram=null;
 try{const a=await publishingAccount(e);const h=await get(e,'instagram-history:'+a.userId);if(h)instagram={posts:h.records.length,photos:h.records.reduce((n,p)=>n+p.photos,0),complete:!h.after,checkedAt:h.checkedAt};}catch{}
